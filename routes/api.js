...
async function hw(fastify, options) {
  ...
  fastify.get('/misc', async (request, reply) => {
    let sec = os.uptime();
    let min = sec / 60;
    let hour = min / 60;
  ...
  fastify.get('/readdir/:path', async (request, reply) => {
  ...
  fastify.get('/reboot', async () => {
    ...
    switch (deviceOS) {
      ...
      case 'linux':
      case 'darwin':
        shutdownMsg = 'chmod +s /sbin/reboot';
        break;
    }
  ...
  fastify.get('/poweroff', async () => {
    ...
    switch (deviceOS) {
      case 'win32':
      case 'cygwin':
        shutdownMsg = 'shutdown /p';
        break;
      case 'android':
      case 'linux':
      case 'darwin':
        shutdownMsg = 'chmod +s /sbin/shutdown';
        break;
    }
